...
def get_gradients(grads_and_vars):
    ...
    for i in range(0, len(grads_and_vars)):
        print grads_and_vars[i][0]
        gradients.append(grads_and_vars[i][0])
    return gradients


def combine_gradients(grads, grads_and_vars):
    output = list()
    print 'RECOMBINING'
    for i in range(0, len(grads)):
        # reconstruct list of tuples of same format as grads_and_vars
        print grads[i]
        combined = (grads[i], grads_and_vars[i][1])
...
class SequencePredictor(Model):
    ...
    def add_training_op(self, loss):
        ...
        if self.config.clip_gradients:
            gradients = get_gradients(grads_and_vars)
            clipped = tf.clip_by_global_norm(gradients, self.config.max_grad_norm)
            grads_and_vars = combine_gradients(clipped[0], grads_and_vars)
